...
主 agent 在生成 `{{HARNESS_MEMORY_DIR}}/plans/project-context.md` 骨架并完成基本资产复制之后，按下列顺序派出三个独立 SubAgent。每个 SubAgent **使用 Claude Agent 工具（General/Agent）独立上下文**加载并执行对应的子能力，主 agent 只持有 orchestration + 用户交互职责，不读 SRS / Design / ATS 全文。
...
- `scripts/check_configs.py` 按 `tech_stack.language` + Design 选加载方式（dotenv / Spring properties / YAML / 系统 env），接口 `python scripts/check_configs.py --task-id <id>`（读 `{{TASK_GET}}` 输出而非 feature-list.json），env 类型查 `os.environ` / file 类型查 `os.path.exists`，缺失打印 `name` + `check_hint`；Exit 0 全存在 / Exit 1 缺失。**加载逻辑硬编码**，不要 `--dotenv` / format 标志。
...
- **`manual-adjust` → 暂停等待用户编辑** — 提示用户直接编辑任务结构（`.harness/blueprint/tasks/iter.json`）；resume 后主 agent **只重跑 features 校验**（任务结构合法性 + srs_trace orphan 检查），不再重分发 SubAgent。
...
- iter-tasks (N features seeded into iter loop)
...
- 任何失败 → 诊断根因，修 `init.sh` / `init.ps1` / `{{HARNESS_MEMORY_DIR}}/notes/env-guide.md` / `scripts/check_configs.py`，重跑
...
- **iter-tasks.json 字段单一写者** — 仅 init-features sub-skill 写入引擎即弃路径；env / bootstrap 只读 `tech_stack`（来自 project-context.md）
