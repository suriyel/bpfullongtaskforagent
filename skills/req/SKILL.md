...
3. **问题与范围挖掘**
   - Lite：L1（聚焦问题与范围，1 轮）
   - Expert：E1（问题框架，参考 `reference/problem-framing.md`）+ E2（增强范围）
4. **功能需求挖掘**
   - Lite：L2（扁平能力轮次）
   - Expert：E3（场景走查，参考 `reference/scenario-walkthrough.md`）+ E4（假设-纠正，参考 `reference/hypothesis-correction.md`）
5. **NFR + 隐藏需求**
   - Lite：L3（合并，1 轮）
...
10. **SRS 合规评审（Step 13）** —— DISPATCH reviewer（加载 `reference/srs-reviewer-prompt.md`）；关卡：所有检查 PASS
11. **呈现并审批 SRS（Step 14）** —— Lite：单一合并；Expert：按章节逐段
12. **保存（Step 15）** —— DISPATCH `long-task-requirements-finalize`（写 `{{HARNESS_MEMORY_DIR}}/plans/srs.md` + 可选 deferred；蓝图模式下不入 git——产物归 harness memory 管理）
...
完成 E1+E2 后用新信息重估 5 信号。若降至 <3（典型：JTBD 明确、单角色确认、无监管暴露）：跳过 E3 走查；E4 直接用 Archetype 批处理（见 `reference/hypothesis-correction.md`）；E10 保留。目标 7–10 轮。防止 3 信号边界项目被锁死完整 Expert 路径。
...
阅读 `reference/problem-framing.md` 并严格执行。
...
阅读 `reference/scenario-walkthrough.md` 并严格执行。

**摘要**：每个主要工作流一次走查（1–3 个工作流）。用户端到端叙述。LLM 抽取显式步骤、隐式步骤、流程缺口、集成点、错误提及。流程缺口追问（受抽取数约束）。

### E4：假设-纠正 [仅 Expert]

阅读 `reference/hypothesis-correction.md` 并严格执行。
...
按 `reference/approval-revise-loop.md` 处理返回。`blocked` 时主 agent 按 `blockers` 驱动 AskUserQuestion（≥3 追溯性缺口逐项决策 / JTBD 未覆盖方面选择），Clarification Addendum 重分发。
...
按 `reference/approval-revise-loop.md` 处理返回。pass 时由主 agent 驱动下列交互（顺序固定）：
...
> **DISPATCH** → 创建独立 SubAgent（使用 General 或 Agent），在 subagent 中加载 `reference/srs-reviewer-prompt.md` 并执行 reviewer
...
按 `reference/approval-revise-loop.md` 处理返回。`fail` 时双轨解决：
...
按 `reference/approval-revise-loop.md` 处理返回。pass 后本节点结束，下游 UCD 由蓝图 DAG 自动推进。
