...
6. **约束、假设、术语表** —— 两条轨道相同
7. **共享质量流水线（Step 7–11）** —— 主 agent 直接执行分类 / EARS / 图表 / 8 属性校验 / 粒度 / 延后候选（详表见 `references/quality-pipeline.md`），按结果驱动 user-input-required AskUserQuestion
8. **单轮模式声明（Step 11b）** —— AskUserQuestion 可选标记 `Single-Round: Yes`
9. **[仅 Expert] 一致性校验（E10）** —— 主 agent 按 `references/alignment-validation.md` 执行 4 项校验（E10a–E10d）
10. **SRS 合规评审（Step 13）** —— DISPATCH reviewer（加载 skill `srs-reviewer`）；关卡：所有检查 PASS
11. **呈现并审批 SRS（Step 14）** —— Lite：单一合并；Expert：按章节逐段
12. **保存（Step 15）** —— 主 agent 直接落盘 `{{HARNESS_MEMORY_DIR}}/plans/<date>-<topic>-srs.md` + 可选 deferred（蓝图模式下不入 git——产物归 harness memory 管理）
...
Lite 挖掘后，进入 Step 7–11（主 agent 直接执行质量流水线）→ Step 11b → Step 13（DISPATCH reviewer，`track=lite` 使 Group P = PASS-SKIPPED）→ Step 14（整段 SRS 单一审批）→ Step 15（主 agent 直接落盘）。下游 UCD 由蓝图 DAG 自动推进。
...
进入 Step 7–11（主 agent 直接执行质量流水线；与 Lite 共享）。

### E10：一致性校验 [仅 Expert]

阅读 `references/alignment-validation.md` 严格执行 4 项校验。

**输入**：从主 agent 已持有的状态读取 `pain_map`、`jtbd`、`workarounds`、`walkthrough_findings`、`hidden_reqs_yes_answers`、`fr_list`、`nfr_list`（从 Step 7-11 draft_sections 中提取）。
...
- **自解（合并入 SRS）**：
  - E10a 追溯性缺口 ≤2 条 → 为每个缺口生成 1 条最小化 FR 或新增 Out-of-Scope 条目（择变更更小者）→ 并入 FR/EXC 列表
  - E10c Pre-mortem 明确属于 FR/NFR 类型的补漏项 → 并入 FR/NFR 列表
- **用户裁决**（直接 AskUserQuestion，不走 blocked）：
  - E10a 追溯性缺口 ≥3 条 → AskUserQuestion 让用户逐项决定"转 FR 还是转 EXC"
  - E10b JTBD = FAIL 且无法从现有 FR 推导 → AskUserQuestion 让用户选择"新增 FR"或"确认 PARTIAL"
- **Open Questions（进 §11，不阻塞）**：
  - E10d 孤儿 FR（无依赖者、无可追溯来源）
  - E10b PARTIAL 裁决（需向用户确认后记入 §11）

**生成 `alignment_summary_text`**（待 Step 15 写入 SRS §1.3）：
...
**PASS 条件**：E10a 缺口 ≤2 已自解 + E10b 为 PASS 或 PARTIAL（已用户确认）+ E10d 孤儿已进 Open Questions。

### E11：SRS Reviewer、呈现、保存

进入 Step 13（DISPATCH reviewer，`track=expert`，Group P 激活）→ Step 14（非平凡项目按章节逐段呈现，不使用单一合并审批）→ Step 15（主 agent 直接落盘，把 `alignment_summary_text` 写入 SRS §1.3）。下游 UCD 由蓝图 DAG 自动推进。

---

## Step 7–11：共享质量流水线（两条轨道）

**Step 10.0 —— 选定 sizing 档位**：
...
主 agent 直接执行流水线，**详细方法论严格按 `references/quality-pipeline.md` 执行**（含 EARS 5 模板、8 属性、反模式、G1-G6 / S1-S4 启发式与阈值）。本节按以下步骤顺序处理：

### A. 分类

把 Lite/Expert 轨道挖掘到的 `raw_requirements` / `raw_nfrs` / `constraints` / `interfaces` / `exclusions` 分配到下列类别：
...
对每条 FR 选模板（Ubiquitous / Event-driven / State-driven / Unwanted behavior / Optional），附加验收标准 / 视觉输出 / 优先级 / 来源；每条 NFR 必须带可度量阈值。**5 模板详表与字段约束见 `references/quality-pipeline.md` §EARS**。
...
按 `references/quality-pipeline.md` §quality-checks 逐需求过 8 属性 + 反模式表 + 完备性交叉检查。自动修复 LLM-FIXABLE 项（措辞、默认 MoSCoW、Given/When/Then 骨架）；修复不了的进入 `user_input_required[]`。

### E. 粒度分析

按 `references/quality-pipeline.md` §granularity 执行 G1-G6 过大检测 + S1-S4 过小检测。决策阈值：

| 候选数 | 动作 |
|---|---|
| 0 | 跳过 |
| 1–3 | 自动应用拆分 / 合并，记录 rationale |
| 4+ | **不自动应用**；进入 `granularity_user_input_required[]` |

### F. 范围契合与延后

- Must 永不延后
- 依赖完整性：FR-X 依赖 FR-Y → 两者同进退
- 候选延后项进 `deferral_candidates[]`，保留完整 EARS + AC（供 Step 15 写 `*-deferred.md`）

### G. 组装 draft_sections

按 `srs_template_path` 读模板，把 A–F 产物填入对应章节，生成 `draft_sections`（内存对象，**不落盘**；Step 15 才落盘）。

### 用户裁决（顺序固定）

A–G 完成后，按下列顺序驱动 AskUserQuestion：

1. `user_input_required[]` — 每条模糊项 AskUserQuestion 收澄清，纳入修正
2. `granularity_user_input_required[]` — 4+ G/S 候选审批
...
主 agent 直接落盘（无需 SubAgent）。准备输入：`approved_srs_sections`（Step 14 审批通过的章节 map）、`deferred_items`（Step 7–11 第 3 步确认的延后清单，可空）、`topic_name`、`single_round_flag`、`alignment_summary_text`（Expert 轨道；Lite 省略）、`srs_template_path`。

**A. 读模板**：从 `srs_template_path` 读 SRS 模板。校验：`.md` 文件且至少含一个 `## ` 标题。
...
**C. 写 frontmatter**：在文件顶部（在首个 `# ` 标题之前）写入：
...
- `alignment_summary_text` 非空 → 写入 SRS §1.3 "Alignment Validation" 字段（若章节已有占位，替换；否则在 §1.3 末尾追加）

**D. 保存 SRS**：`{{HARNESS_MEMORY_DIR}}/plans/<YYYY-MM-DD>-<topic_name>-srs.md`。

路径冲突处理：文件已存在 → AskUserQuestion 让用户决定覆盖 / 改 topic_name / 中止。

校验：`topic_name` 必须 `[a-z0-9-]`；不合法直接 AskUserQuestion 让用户改名。

**E. 保存 deferred backlog**：若 `deferred_items` 非空：
- 保存到 `{{HARNESS_MEMORY_DIR}}/plans/<YYYY-MM-DD>-<topic_name>-deferred.md`
- 每条保留原 EARS + AC 字段
- 顶部写 `Source: <srs_path>` + `Date: YYYY-MM-DD`

若 `deferred_items` 为空，跳过本步。

**F. 完成**：蓝图模式下产物落 `.harness/memory/plans/`，归 harness memory 管理，**不入用户项目 git**。SRS approved 事件由蓝图引擎 `.harness/blueprint/runs/<runId>.jsonl` 自动记录，无需 commit。本节点结束，下游 UCD 由蓝图 DAG 自动推进（`gate_srs` 硬门会校验 SRS 落盘是否合法）。
