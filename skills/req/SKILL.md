...
10. **SRS 合规评审（Step 13）** —— DISPATCH reviewer（加载 `references/srs-reviewer-prompt.md`）；关卡：所有检查 PASS
...
> **DISPATCH** → 创建独立 SubAgent（{{AGENT}}），在 subagent 中加载 `references/srs-reviewer-prompt.md` 并执行 reviewer
