...
**核心原则：** Feature Design 子步骤在**独立 SubAgent**中运行（`feature-design`）。主 Agent 仅负责分发并消费 **Structured Return Contract** —— 契约与 DISPATCH 语法参见 `{{SHARE-REFERENCE}}/structured-return-contract.md`（若存在；否则使用本文末 § Structured Return Contract 内嵌定义）；SubAgent 返回按 `{{SHARE-REFERENCE}}/approval-revise-loop.md` 处理。
...
- `task` = 任务对象本身
- 读 `{{HARNESS_MEMORY_DIR}}/plans/design.md` § 架构（§1）+ `task` 对应的 `§2.N` 子节（按"文档查询协议"通过 Read offset/limit 定位）
- 读 `{{HARNESS_MEMORY_DIR}}/plans/srs.md` 中 `task.srs_trace` 指向的所有 FR-xxx 子节
- 读 `{{HARNESS_MEMORY_DIR}}/notes/env-guide.md §4`（如存在）
- 若 `task.ui == true` 且 `{{HARNESS_MEMORY_DIR}}/plans/ucd.md` 存在：读 UCD 样式指南相关章节
- `git log --oneline -10` 取最近 commit 上下文
- 在 `{{HARNESS_MEMORY_DIR}}/notes/task-progress.md` 当前特性标题下记录：task.id / title / design_section 行号 / srs_section 行号
...
仅当 `task.required_configs[]` 含连接串键（URL / URI / DSN / CONNECTION / HOST / PORT / ENDPOINT）时执行；否则跳过。
...
> **DISPATCH** → 创建独立 SubAgent（{{AGENT}}），在 subagent 中加载并执行 skill `feature-design`
> **input**: `feature_id`（取自 `{{TASK_GET}}`）, `task`（完整任务对象，含 srs_trace / tech_stack / ui / category / required_configs 等业务字段）, `design_section=<行号起止>`, `srs_section=<FR-xxx 行号起止>`, `ucd_section=<仅 ui:true>`, `working_dir`
...
**5b-ui. 视觉渲染契约抽查（仅 `task.ui == true`）**：读取生成文档中的 `## Visual Rendering Contract` 章节并校验：
...
- **SubAgent 不可协商** —— `feature-design` 必须通过 {{AGENT}} 分发
