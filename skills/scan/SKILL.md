...
**读取执行规则**：`references/scan-execution.md`（含 Commit Conventions 分析章节，作为本 SKILL 流程之外的执行补充）。
...
> 注：用户启动蓝图时输入的原始诉求由后端在 `launchRun` 时直接落盘到 `{{HARNESS_MEMORY_DIR}}/intent/user-original-intent.md`（由框架在系统提示词中说明），scan skill **不再**承担该文件的写入，直接专注扫描代码库约定。
...
> Auto-generated by scan node on YYYY-MM-DD.
...
---
*Scanner: scan node | Depth: [level] | Files sampled: N*
```
