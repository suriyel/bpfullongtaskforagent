---
name: feature-design
description: "当 wd 节点 DISPATCH 时使用 — 在独立 SubAgent 中产出特性级详细设计（接口契约 / 实现摘要 / 算法伪码 / Mermaid 图 / 测试清单 / 视觉渲染契约），按 Structured Return Contract 返回 pass/fail/blocked；bugfix 类目精简模式"
---

# Feature-Level Detailed Design — SubAgent 执行 Skill
...
1. **Feature 对象**（来自当前 task；调 `{{TASK_GET}}` 获取） — ID、title、description、srs_trace、ui flag、dependencies、priority（如有 verification_steps）
2. **系统设计章节** — 设计文档中完整的 §2.N（读整个子章节，不要用 grep）
3. **SRS 需求** — SRS 文档中完整的 FR-xxx
4. **UCD 章节**（若 `"ui": true`） — UCD 文档中的 component/page 提示词
5. **Constraints & assumptions**（来自 task 的 `constraints[]` / `assumptions[]` 字段）
...
- **任一歧义有高影响**（影响 Interface Contract 签名、Test Inventory 预期结果或跨特性契约） **或无合理的建议解释** → `status` 设为 `blocked`。为每条高影响歧义追加一条 blockers[] 条目，严格使用前缀约定（见下表）。**不**要进入 Step 1c — orchestrator 会按 `{{SHARE-REFERENCE}}/approval-revise-loop.md` 收集用户裁决并以 Clarification Addendum 重分发。
...
4. orchestrator（蓝图 wd 节点）按 `{{SHARE-REFERENCE}}/approval-revise-loop.md` 处理，向用户呈 A/B 选项
...
**与 TDD 的关系**：本表是蓝图 iter loop 内 red 节点的**主要输入**。red 以本表为起点，并可依其自身 Rule 1-5（类别覆盖、断言质量、真实测试要求）添加测试。Test Inventory 提供设计驱动场景；TDD 添加编码过程中发现的实现驱动场景。
...
与 `{{SHARE-REFERENCE}}/structured-return-contract.md` 中的统一契约对齐。严格按此格式返回：

```markdown
## SubAgent Result: feature-design

**status**: pass | fail | blocked
**artifacts_written**: ["{{HARNESS_MEMORY_DIR}}/notes/feature-<id>-design.md"] (omit if status=blocked before any draft was written)
**next_step_input**: {
