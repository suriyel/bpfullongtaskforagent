...
在 Finalize 阶段（finalize 节点 Step 2）作为 SubAgent 被分发。接收：
- `tasks`：来自 `{{TASKS_GET}}` 的完整任务清单 JSON
- SRS 文档路径：`{{HARNESS_MEMORY_DIR}}/plans/srs.md`
- Design 文档路径：`{{HARNESS_MEMORY_DIR}}/plans/design.md`
- UCD 样式指南路径：`{{HARNESS_MEMORY_DIR}}/plans/ucd.md`（仅 UI 项目；不存在传 `"none"`）
- 来自任务清单的 `tech_stack`（顶层字段，从 tasks 提取）
- 工作目录（项目根）
...
1. 调用 `{{TASKS_GET}}`——提取所有 `status: "passing"` 且 `deprecated` 不为 `true` 的任务
2. 读取 `{{HARNESS_MEMORY_DIR}}/plans/srs.md`——理解需求描述、用户画像、验收标准
3. 读取 `{{HARNESS_MEMORY_DIR}}/plans/design.md`——理解架构、公有 API 表面、模块边界
4. 读取 `{{HARNESS_MEMORY_DIR}}/plans/ucd.md`（若存在 UI 特性）——理解 UI 流程
...
- **遵循项目语言**——使用 `{{TASKS_GET}}` 返回的 `tech_stack` 字段中的语言
