...
**SRS 引用**: {{HARNESS_MEMORY_DIR}}/plans/srs.md
...
> *本节在设计时从 `{{HARNESS_MEMORY_DIR}}/notes/rules/` 自动填充（适用于有现有代码库的存量项目）。对于新建项目，保留各子节及空表格 -- 不要将整节标记为"不适用"。下游技能始终读取 §11；空表格表示"无约束"，无需条件逻辑。*
